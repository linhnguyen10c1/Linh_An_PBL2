...
void Menu_general_doctor(long long ID_doctor, LinkedList<Record> &record_list, LinkedList<Doctor> &doctor_list)
{
	...
	do
	{
		...
		case 3:
		{
			...
			cin >> ID_checking;// trước tiên phải kiểm tra checking đó có  tồn tại hay không
			cin.ignore();
			int choice;
			do
			{
          ...
          write_data_to_file(doctor_list, "doctors.txt");
					record_list.update_from_general_doctor(ID_checking, x);
...
void Menu_detail_doctor(long long ID_doctor, LinkedList<Record> &record_list, LinkedList<Doctor> &doctor_list)
{
	...
	do
	{
		...
		case 1:
		{
			testing_list.display_list_waiting_testing(ID_doctor);
			break;
		}
		case 2:
		{
			long long ID_checking;
			cout << "ID_checking you want testing: ";
			cin >> ID_checking;
			cin.ignore();
			int choice;
			do
			{
				...
				case 1:
				{
					record_list.search(ID_checking);
					break;
				}
				case 2:
				{
          ...
          write_data_to_file(doctor_list, "doctors.txt");
					//record_list.update_from_general_doctor(ID_checking, x);
					testing_list.update_data_from_detail_doctor(ID_checking);
					write_data_to_file(testing_list, "testings.txt");
					break;
				}
				case 0:
